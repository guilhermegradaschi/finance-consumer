...
import { parseXmlToHash, extractInfNfe } from '../../common/utils/xml-parser.util';
...
export class InvoiceCreatorService {
  ...
  async create(
    ...
    createdInvoiceNumbers: Set<string>,
    ...
    try {
      ...
      if (await this.duplicationService.isDuplicate(attrs.invoiceNumber, attrs.date, attrs.buyerCnpj, attrs.sellerCnpj)) {
      ...
      const referenceDate = await this.periodStatusService.adjustToOpenPeriod(
        attrs.date,
        seller.headquarter.id,
      );

      const buyerAssociation = await this.buyerAssociationService.associate(
      ...
      const invoice = this.invoiceRepo.create({
        ...
        ignoredReason: alreadyBilled
          ? InvoiceIgnoredReason.ALREADY_BILLED
          : InvoiceIgnoredReason.NOT_IGNORED,
