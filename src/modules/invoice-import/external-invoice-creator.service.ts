...
import { parseXmlToHash, extractInfNfe, safeGetString, ensureArray } from '../../common/utils/xml-parser.util';
import { cfopToOperation, isDevolucaoAssignDefault, isDevolucao } from '../../common/constants/invoices-code-operations';
...
export class ExternalInvoiceCreatorService {
  ...
  async create(
    ...
    const firstProd = firstDet ? ((firstDet['prod'] ?? {}) as Record<string, unknown>) : ({} as Record<string, unknown>);
    ...
    const orderNumber = compra && typeof compra === 'object'
      ? safeGetString(compra as Record<string, unknown>, 'xPed')
      : null;
