...
import { extractChaveAcessoFromXml } from '@shared/utils/xml.util';
...
export class ImapImporterService {
  ...
  private async processEmail(email: ImapEmailPayload, invoiceImportId: string): Promise<void> {
    const source =
      email.from === 'notificacao@grupoamicci.com.br' ? ExternalInvoiceSource.BUYER : ExternalInvoiceSource.IMAP;
    ...
    const result = await this.externalInvoiceCreator.create(email.accessKey, email.xml, source, invoiceImportId);
    ...
    await this.invoiceImportLogRepo.save(
      this.invoiceImportLogRepo.create({
        ...
        log: {
          ...
          attachments: [
            {
              filename: `${email.accessKey}.xml`,
              status: result.created ? 'success' : 'ignored',
              access_key: email.accessKey,
            },
          ],
        },
        metadata: email.metadata,
