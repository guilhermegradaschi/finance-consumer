  ...
  async persist(event: NfValidatedEventDto): Promise<void> {
    ...
    try {
      const nf = queryRunner.manager.create(NotaFiscal, {
        ...
        dataAutorizacao: event.sefazValidation?.dataAutorizacao
          ? new Date(event.sefazValidation.dataAutorizacao)
          : null,
      ...
      throw new RetryableException(`Persistence failed: ${(error as Error).message}`, 'NF010', {
        chaveAcesso: event.chaveAcesso,
      });
    } finally {
